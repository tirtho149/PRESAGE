...
def to_classes(data,text_type):
    ...
    # Get Taxon field names (excluding methods and private attributes)
    taxon_fields = {field.name for field in dataclasses.fields(Taxon)}
    # Remove columns that are not Taxon fields
    data_view = data.drop(columns=list(set(data.keys()) - taxon_fields), errors='ignore')
