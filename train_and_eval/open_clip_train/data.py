...
def get_wds_dataset(args, preprocess_img, is_train, epoch=0, floor=False, tokenizer=None):
    ...
    if args.text_type == 'random':
        ...
        pipeline.extend([
            ...
            # PlantSwarm-on-Bugwood: only two text fields per sample.
            # `taxon`   -> short label ("Tomato Early blight") for the taxonomy projector.
            # `caption` -> long KB-derived description for the caption projector.
            wds.rename(image="jpg;png;jpeg;webp", taxon="taxon.txt", caption="caption.txt"),
            wds.map_dict(image=preprocess_img, taxon=lambda taxon: tokenizer(taxon)[0], caption=lambda caption: tokenizer(caption)[0]),
            wds.to_tuple("image", "taxon", "caption"),
