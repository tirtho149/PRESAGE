#!/bin/bash
# scripts/submit_pathomeood_matrix.sh
# ============================================================================
# Submit every variant in scripts/pathomeood_variants.sh as a separate SLURM
...
#
# Usage:
#   bash scripts/submit_pathomeood_matrix.sh                  # background sbatch
#   PATHOME_WAIT=1 bash scripts/submit_pathomeood_matrix.sh   # block per job
# ============================================================================
set -euo pipefail
...
if [ "${PATHOME_SKIP_CAPTIONS:-0}" != "1" ]; then
  for v in "${PATHOMEOOD_VARIANTS[@]}"; do
    ...
    if [ ! -f "$capt_out" ] && [ ! -f "${capt_out%.parquet}.tsv" ]; then
      ...
      python scripts/build_pathomeood_captions.py --strategy "$STRATEGY" --crop "$CROP" --out "$capt_out"
    fi
    if [ ! -d "$shards_root/train" ]; then
      caps_path="$capt_out"
      [ -f "$caps_path" ] || caps_path="${capt_out%.parquet}.tsv"
      echo "[matrix] building shards: $STRATEGY"
      python scripts/build_pathomeood_shards.py --captions "$caps_path" --out-dir "$shards_root"
    fi
  done
fi
...
for v in "${PATHOMEOOD_VARIANTS[@]}"; do
    pathomeood_parse_variant "$v"
    cmd=(sbatch $WAIT_FLAG --job-name="biocap-$VARIANT_TAG" --export=ALL,VARIANT="$VARIANT_TAG",CROP="$CROP" scripts/submit_pathomeood_train.sh)
