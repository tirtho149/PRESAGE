#!/bin/bash
# ============================================================================
# scripts/sh_01_phase0_local_copy.sh      STEP 1 — LOCAL (Corn copy)
# ============================================================================
# Copy of sh_01_phase0_local.sh that defaults to building the canonical KB
# for Corn, i.e. it produces artifacts/pathome_kb/Corn/final_registry.json.
#
...
#   For each crop in $CROPS (defaults to "Corn" in this copy):
...
#                          default "Corn" — baked in for this copy
#   RESUME_FROM            optional: discovery | extraction | reconciliation
#                          when set, each crop resumes from this stage and
#                          reuses the cached upstream artefacts. Use
#                          "reconciliation" to re-run ONLY Stage 3 against an
#                          existing discovery_results.json + raw_extractions.json
#                          (cheap; isolates a failing reconciliation). Default
#                          empty = run all three stages from scratch.
...
# Force UTF-8 for Python stdout/stderr. pathome_kb prints non-ASCII
# characters (e.g. the "->" arrow in the reconciliation stage); on Windows
# the console defaults to cp1252 and print() raises UnicodeEncodeError,
# which aborts reconciliation before final_registry.json is ever written.
export PYTHONUTF8=1
...
# Validate RESUME_FROM early — pathome_kb only accepts these three stages.
case "$RESUME_FROM" in
  ""|discovery|extraction|reconciliation) ;;
  *) echo "ERROR: RESUME_FROM must be discovery, extraction, reconciliation, or empty (got '$RESUME_FROM')"
     exit 2;;
esac
...
echo "================================================================="
echo " STEP 1 — Phase 0 canonical KB (LOCAL, Claude) — Corn copy"
echo "================================================================="
echo "  CROPS                : ${PATHOME_ONLY_CROPS:-(all 197)}"
echo "  RESUME_FROM          : ${RESUME_FROM:-(none — run all stages)}"
...
cmd=("$PY" -m pathome_kb --csv "$PATHOME_USABLE_CSV")
if [ -n "$PATHOME_ONLY_CROPS" ]; then
  echo "  --only-crops $PATHOME_ONLY_CROPS"
  cmd+=(--only-crops "$PATHOME_ONLY_CROPS")
fi
if [ -n "$RESUME_FROM" ]; then
  echo "  --resume-from $RESUME_FROM (reusing cached upstream artefacts)"
