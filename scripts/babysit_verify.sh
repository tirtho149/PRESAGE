#!/usr/bin/env bash
# Poll the verification array; log progress (cited-delta count) until done.
JID=$(cat /work/mech-ai-scratch/tirtho/PRESAGE/verify_array_jobid.txt)
cd /work/mech-ai-scratch/tirtho/PRESAGE
count_cited(){ python - <<'PY'
import json,glob,os
c=t=0
...
while squeue -j "$JID" -h -t all 2>/dev/null | grep -qE "R|PD|CG"; do
  ...
  echo "[$(date +%H:%M)] running=$run pending=$pd completed_tasks=$done | $(count_cited)"
  sleep 180
done
echo "=== ARRAY DONE $(date) ==="
