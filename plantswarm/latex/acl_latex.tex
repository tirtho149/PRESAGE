...
\textbf{\pathomedb}: a five-layer mechanistic
cross-crop pathogen knowledge base whose regional
epidemiology layer is derived empirically from
observation density rather than expert curation,
validated against EPPO historical outbreak records
at Pearson $r{\geq}0.71$.
...
Five-layer mechanistic cross-crop pathogen
knowledge base; Layer~3 empirically derived
from \bugwood GPS observation density,
...
\subsection{Training: Bugwood Field Images}

\bugwood~\citep{bugwood} provides geo-tagged
field disease photographs with GPS precision,
species-level labels, and capture timestamps.
We select $10$ images per disease class under
strict quality filters: GPS precision ${\leq}10$km,
species-level disease annotation, resolution
${\geq}512{\times}512$, single dominant subject
with diagnostic region occupying ${\geq}25\%$ of
frame, no severe compression artifacts. From
passing candidates, $10$ images per class are
selected by $k$-medoids ($k{=}10$) diversity
sampling on CLIP~\citep{radford2021learning}
embeddings, covering early, mid, and late stage
presentations and multiple geographic origins.

From the $10$ images per disease class we make
a hard split \emph{before any} \plantswarm
run or \pathomedb construction:
\begin{itemize}
  \item \textbf{7 images per class} $\to$ \plantswarm
    trace generation ($30$ runs per image;
    $7 \times 26 \times 30 = 5{,}460$ traces).
  \item \textbf{3 images per class} $\to$ \pathomedb
    Layer~5 reference library
    ($3 \times 26 = 78$ geo-tagged field references),
    one per disease stage (early/mid/late).
\end{itemize}
Total unique images used in any form of training
or knowledge base construction: $\mathbf{260}$.
...
length $L$, backtrack count $B$, early-termination
flag, prediction entropy $H_{\hat{y}}$, GPS
coordinates $(\lambda, \phi)$, capture
timestamp, FAO agro-ecological zone code,
and correctness label. GPS and AEZ code
feed \pathomedb Layer~3 construction directly.
...
\pathomedb is a five-layer mechanistic
knowledge graph. Each layer serves a distinct
function; together they provide \observe with
the visual, mechanistic, and geospatial context
that $182$ training images alone cannot supply.

\subsection{Layer 1: Mechanistic Infection Pathway}

For each pathogen, Layer~1 encodes the causal
cascade from entry to symptom. This is the
source of cross-crop transferability: enzymatic
mechanisms are crop-agnostic. \textit{Colletotrichum}
secretes the same cutinase-pectinase-cellulase
cascade on mango and soybean, producing the
same circular lesion from radial pectinase
diffusion (Step~3) and the same orange acervuli
from carotenoid conidia (Step~4). An agent
that understands Step~4 can diagnose any host.

Each step records: entry mechanism and trigger
conditions; features that \emph{increase} and
\emph{decrease}; the causal explanation for
each visual feature; and the epistemic
implication for \observe---Step~2 features
imply high $\epsilon_t$ (more evidence would
help), Step~4 orange sporulation implies
immediate early termination.

\subsection{Layer 2: Cross-Crop Manifestation}

Layer~2 maps crop-agnostic mechanisms to
host-specific presentations: lesion diameter
(mango $30$mm, soybean $15$mm, strawberry
$8$mm), sporulation timing, dominant tissue
affected, color shift from host pigmentation
baseline, and confusion pairs per host.
The $12$ unseen \pv disease classes are
diagnosed through Layers~1 and~2 alone:
if a disease maps to a known pathogen
in a known crop category, the mechanistic
pathway provides sufficient routing template.

\subsection{Layer 3: Geospatially-Grounded Regional Epidemiology}
\label{sec:layer3}

Layer~3 is \pathomedb's primary methodological
contribution. Monthly disease prevalence by
FAO agro-ecological zone is estimated from
\bugwood observation density
(Eq.~\ref{eq:density}). Regions with
$<3$ GPS observations in an AEZ-month
cell are marked \texttt{sparse} and excluded
from prior computation; \observe falls back
to the global pathogen prior. The resulting
monthly risk profiles are validated against
EPPO historical records at Pearson
$r{\geq}0.71$ (P7).

Layer~3 additionally records:
\textit{co-occurrence pairs}---diseases
observed within $50$km of each other in
\bugwood, flagged as potential regional
confusion pairs and verified against
extension service literature;
\textit{variety-resistance profiles}
from national agriculture ministry reports;
\textit{treatment availability} by region
(first/second-line options, access level,
cost per hectare in local currency);
and a \textit{camera quality tag} that
adjusts \observe's $\alpha_t$ threshold---smallholder
smartphones produce images with structurally
higher aleatoric uncertainty than
laboratory cameras.

\subsection{Layer 4: Diagnostic Decision Graph}

Layer~4 encodes expert diagnostic reasoning
as a directed acyclic graph (NetworkX DiGraph)
with six mandatory branches in order: host,
plant part, symptom category, progression
direction, pathogen signs, environment.
Angular vs.\ circular lesion margin is the
single most informative visual feature
separating bacterial from fungal etiology
and is a hard branch condition. Terminal
conditions enable early routing: orange
sporulation is definitive for
\textit{Colletotrichum} regardless of host.

Each graph node carries the responsible agent,
visual features required for branch resolution,
posterior updates, and the targeted
re-observation instruction injected into
$\mathcal{C}_{t+1}$ when \observe forces
a backtrack. Backtracking is purposeful:
\observe injects ``examine lesion margins for
orange or salmon masses specifically'' rather
than a generic retry.

\subsection{Layer 5: Geo-Tagged Reference Library}
\label{sec:layer5}

\textbf{Source.} The $3$ images per disease class
held out from \bugwood ($78$ total) are staged
using Qwen2.5-VL-7B prompted with Layer~1
descriptions (five-sample majority vote;
$<60\%$ agreement $\to$ flagged uncertain,
excluded). These are the \textbf{only} reference
images in \pathomedb: no PlantVillage or
PlantWild images are used, ensuring zero
data leakage into either evaluation set.

\textbf{Geo-weighted retrieval.} At inference,
\observe retrieves top-3 references by:
\begin{equation}
  \text{score}(q, r_i) =
...
$\phi$ climate-zone vectors from GPS.
A query from Nigeria retrieves references
from similar humid-tropical field conditions
rather than visually similar but climatically
distant Himalayan images.
FAISS~\citep{johnson2019billion} indexes
precomputed $512$-dim embeddings;
retrieval latency $<1$ms.
...
\section{Bugwood Image Selection}
\label{app:selection}

Hard filters: GPS precision ${\leq}10$km,
species-level disease label, resolution
${\geq}512{\times}512$, single dominant
subject with diagnostic region ${\geq}25\%$
of frame, no severe JPEG compression
($<60$ quality factor). From candidates
passing all filters, $k$-medoids ($k{=}10$,
initialized by $k$-means++) on CLIP ViT-B/32
embeddings selects the maximally diverse
$10$ images per class. This ensures early,
mid, and late stage presentations are covered
and multiple geographic origins are represented
within the $10$.

Of the $10$ images per class: $7$ feed
\plantswarm ($30$ runs each, temperature
$T{=}0.9$, max path length $15$ steps,
backtrack budget $1$ per episode); $3$ are
held as \pathomedb Layer~5 references
(one per disease stage, staged by Qwen2.5-VL-7B
prompted with Layer~1 pathway descriptions,
five-sample majority vote).

\section{Layer 3 Construction Details}
\label{app:layer3}

GPS coordinates from $182$ trace images are
mapped to FAO agro-ecological zones using
the FAO shapefile at $0.5^\circ$ resolution.
AEZ-month cells with $<3$ observations are
marked \texttt{sparse} and excluded from
$\hat{P}(d \mid r, \sigma)$; \observe queries
the global pathogen prior for sparse cells.
Risk tiers: \textit{critical} (top quartile
by count density), \textit{high} (50--75th),
\textit{medium} (25--50th), \textit{low} ($<25$th).
Validation uses Pearson $r$ over shared
(disease, AEZ, month) triples in both
\pathomedb and the EPPO database; five
disease classes are held out from trace
generation for this validation.
