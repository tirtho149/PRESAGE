...
![Phase 0R organ-routed decision tree — 5-act walkthrough](docs/assets/swarm_flow.gif)

*A slow (2 fps) five-act walkthrough of ONE (crop, disease, state)
pass through the organ-routed decision tree. Generated by
`docs/assets/render_swarm_flow.py`.
**Act 1** sets up the static context — the canonical
`visual_symptoms` slice + ONE Bugwood field photo (one photo = one
organ, so we route instead of running everything).
**Act 2** is `OrganDetectionAgent`: one visual call scans the
candidate organs and locks onto the dominant one (`leaf` in the demo)
— pure visual triage, no KB, no diagnosis.
**Act 3** is routing: `route_for_organ('leaf')` activates ONLY the
leaf branch — 8 deep leaf specialists + `ConcentricPattern` + the
always-on cross-cutters (`ColorPalette`, `Severity`, `LookAlikeCoT`,
`Sporulation`) = 13 active agents. The other 7 branches stay
**dormant** for this image.
**Act 4** is the deep dive: the 13 routed agents fan out (round 1),
write deltas to the running log, then a blackboard round-2 refines
with `SUPPORT` / `CHALLENGE` among the active agents only.
**Act 5** runs `VisualDiagnosisAgent` consolidation → cross-pass
K-of-N agreement → Claude web verifier → conservative merge into
